...
from helper import generate_periodic_profiles, override_component_attrs
...
def add_carrier_buses(n, carrier, nodes=None):
    ...
    # Assume some (potentially low, for gas) storage capital costs, so
    # the model doesn't build too much storage.
    capital_cost = costs["investment"].get(carrier + " storage", 0)
    n.madd(
        "Store",
        nodes + " Store",
        bus=nodes,
...
def remove_non_electric_buses(n):
    ...
    """
    print(
        "drop buses from PyPSA-Eur with carrier: ",
        n.buses[~n.buses.carrier.isin(["AC", "DC"])].carrier.unique(),
    )
...
def add_co2limit(n, countries, Nyears=1.0, limit=0.0):
...
def insert_electricity_distribution_grid(n, costs):
    ...
    print(
        "Inserting electricity distribution grid with investment cost factor of",
...
def add_industry(n, costs):
    ...
    p_set = (
        ...
        * pop_weighted_energy_totals.loc[nodes, all_navigation].sum(axis=1)
        * 1e6
        * efficiency
        / 8760
    )
    ...
    if shipping_hydrogen_share < 1:
        ...
        co2 = (
            shipping_oil_share
            * pop_weighted_energy_totals.loc[nodes, all_navigation].sum().sum()
            * 1e6
            / 8760
            * costs.at["oil", "CO2 intensity"]
        )
...
def set_temporal_aggregation(n, opts, solver_name, drop_leap_day):
    """
...
if __name__ == "__main__":
    ...
    if "nuclear" in options["conventional_generation"]:
        nuclear_caps = n.generators[n.generators.carrier == "nuclear"][
            ["p_nom", "p_nom_min", "p_max_pu", "p_nom_extendable"]
        ]
